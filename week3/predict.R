...
df_2015 <- trips %>% group_by(ymd) %>% dplyr::summarise(total_trips = n())  
df_2015 <- inner_join(df, weather_2015, "ymd") 
View(weather)
...
df_2015 <- mutate(tmax.x = tmax, df_2015, is_hot = (tmax*10) > 80,
...
############################3
pred_2015 <- predict(lm.fit8, df_2015)
